    ...
    while let Some(block) = stream.recv().await {
        ...
        for outcome in &block.outcomes {
            match outcome {
                near_indexer::Outcome::Transaction(execution_outcome) => {
                    diesel::update(
                        schema::transactions::table.filter(
                            schema::transactions::hash.eq(execution_outcome.id.to_string()),
                        ),
                    )
                    .set((
                        schema::transactions::dsl::receipt_id.eq(execution_outcome
                            .outcome
                            .receipt_ids
                            .first()
                            .unwrap()
                            .to_string()),
                        schema::transactions::dsl::receipt_conversion_gas_burnt.eq(Some(
                            BigDecimal::from_u64(execution_outcome.outcome.gas_burnt)
                                .unwrap_or(0u64.into()),
                        )),
                        schema::transactions::dsl::receipt_conversion_tokens_burnt.eq(Some(
                            BigDecimal::from_u128(execution_outcome.outcome.tokens_burnt)
                                .unwrap_or(0u64.into()),
                        )),
                    ))
                    .execute_async(&pool)
        ...
        for chunk in block.chunks {
            for receipt in chunk.receipts {
                ...
                let receipt_exists = dsl::select(dsl::exists(
                    schema::receipts::table
                        .filter(schema::receipts::receipt_id.eq(receipt_id.clone())),
                ))
                ...
                if receipt_exists {
                    ...
                    diesel::update(
                        schema::receipts::table.filter(schema::receipts::receipt_id.eq(receipt_id)),
                    )
                    .set((
                    ...
                    diesel::insert_into(schema::receipts::table)
                        .values(models::Receipt::from_receipt(&receipt))
                        .execute_async(&pool)
                        .await
                        .unwrap();
                }
                    ...
                    near_indexer::near_primitives::views::ReceiptEnumView::Action {
                        ...
                        for (i, action) in actions.iter().enumerate() {
                            diesel::insert_into(schema::actions::table)
                                .values(models::Action::from_action(
                                    receipt.receipt_id.to_string().clone(),
                                    i as i32,
                                    action,
                                ))
                            ...
                            match action {
                                near_indexer::near_primitives::views::ActionView::CreateAccount => {
                                    match diesel::insert_into(schema::accounts::table)
                                        .values(models::Account::new(
                                            receipt.receiver_id.to_string().clone(),
                                            i as i32,
                                            receipt.receipt_id.to_string().clone(),
                                            BigDecimal::from_u64(block.block.header.timestamp)
                                                .unwrap_or(0.into()),
                                        ))
                                        .execute_async(&pool)
                                        .await
